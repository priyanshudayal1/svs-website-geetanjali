...
function Footer() {
  return (
    <footer className="bg-gradient-to-b from-[#f1f0f4] to-[#fff8f3] px-6 pt-12 pb-14 max-[720px]:px-4 max-[720px]:pt-8 max-[720px]:pb-10">
      <div className="mx-auto max-w-[1240px] overflow-hidden rounded-lg border border-[#f45a2242] bg-[#151719] text-white shadow-[0_20px_42px_rgb(35_25_20_/_18%)]">
        <div className="grid grid-cols-[minmax(220px,1.5fr)_repeat(3,minmax(130px,0.8fr))_minmax(180px,1fr)] gap-11 px-14 pt-[34px] pb-[30px] max-[1080px]:grid-cols-[1.4fr_1fr_1fr] max-[720px]:grid-cols-1 max-[720px]:gap-7 max-[720px]:p-7 max-[720px]:px-[22px]">
          <div>
            <img
              className="mb-4 block h-[54px] w-auto object-contain"
              src={logo}
              alt="SVS Food"
            />
            <p className="m-0 mb-2 text-sm leading-normal text-[#d7d7d7]">
            ...
            <div className="mt-[22px] flex gap-3">
              {socialLinks.map(({ icon: Icon, label }) => (
                <a
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/20 text-[#f45a22] no-underline transition hover:border-[#f45a22] hover:bg-[#f45a22] hover:text-white"
                  href="/"
                  aria-label={label}
                  key={label}
                >
          ...
          {footerGroups.map((group) => (
            <nav className="flex flex-col gap-3" key={group.title}>
              ...
              {group.links.map((link) => (
                <Link
                  className="text-sm text-[#d8d8d8] no-underline transition hover:text-[#ff7a38]"
                  key={link}
                  to={footerPath(link)}
                >
                  {link}
          ...
          <div className="flex flex-col gap-3 max-[1080px]:col-span-2 max-[720px]:col-auto">
            ...
            <a
              className="inline-flex min-h-12 items-center gap-2.5 rounded-[7px] border border-white/50 px-[13px] py-1.5 text-white no-underline transition hover:-translate-y-px hover:border-[#f45a22]"
              href="/"
            >
              <FaGooglePlay
                className="h-6 w-6 text-[#f45a22]"
                aria-hidden="true"
              />
        ...
        <div className="flex min-h-[52px] items-center justify-between border-t border-white/10 px-14 text-[#cfcfcf] max-[720px]:items-start max-[720px]:gap-4 max-[720px]:px-[22px] max-[720px]:py-[18px]">
          <p className="m-0 text-[13px] font-bold">
            &copy; 2024 SVS Food. All rights reserved.
          </p>
          <FiPlay
            className="h-5 w-5 -rotate-45 text-[#f45a22]"
            aria-hidden="true"
          />
        </div>
      </div>
